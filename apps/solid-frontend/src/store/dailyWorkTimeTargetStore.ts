...
const [dailyWorkTimeTargets] = createStore({
    get getById() {
        return async (id: string) => {
            const result = await directus.items("DailyWorkTimeTarget").readOne(id);
            return result;
        };
    },
    get getForDate() {
        return async (date: Date, userId = "$CURRENT_USER") => {
            const currentBlock = await directus.items("WorkTimeTargetBlock").readByQuery({
                filter: {
                    workerId: {
                        _eq: userId,
                    },
            ...
            const result = await directus.items("DailyWorkTimeTarget").readByQuery({
                filter: {
                    id: {
                        _in:
                            currentBlock.data?.flatMap((block) => block?.DailyWorkTimeTargets) ??
                            [],
                    },
                    dayOfWeek: {
    ...
    get getForDateRange() {
        return async (start: Date, end: Date, userId = "$CURRENT_USER") => {
            const blocks = await directus.items("WorkTimeTargetBlock").readByQuery({
                filter: {
                    workerId: {
                        _eq: userId,
                    },
            ...
            const dailies = await directus.items("DailyWorkTimeTarget").readByQuery({
                filter: {
                    id: {
                        _in: dailiesIds,
                    },
            ...
            for (let day = 0; day <= dayRange; day++) {
                ...
                if (block) {
                    const daily = dailies.data
                        ...
                        .filter((d) => d.dayOfWeek === dayjs(start).add(day, "day").isoWeekday());
                    if (daily)
